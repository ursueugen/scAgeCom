...
# Interactive cell-cell communication graphs
...
# ct = get_celltypes_enrichment(ora_ct)
# getClass(facs$Liver)
G = construct_graph(ora_ct, 'Dummy', dt_filtered=facs$Liver@cci_table_filtered)
G = setup_graph(G, use_adjpval=TRUE, disperse=FALSE)

nodes = data.table(as_data_frame(G, what='vertices'))
edges = data.table(as_data_frame(G, what='edges'))
igraph_layout = G$layout

# Global node config
NODES_SHAPE = 'square'
NODES_COLOR_CONFIG = list(
  background='gray',
  border='darkblue', 
  highlight=list(background='lightblue', border='yellow'), 
  hover=list(background='lightred', border='green')
)

# Global edges config


# Individual nodes config
nodes[,
      'id' := name][,
      'label' := name][,
      'group' := vertex_types][,
      ...
      'level' := ifelse(vertex_types == TRUE, 1, 2)
]
# nodes

# Individual edges config
edges[,
...
network_global = visNetwork(
    ...
    footer='Graph',
    background='white')
nodes_global = . %>% visNodes(shape=NODES_SHAPE, physics=FALSE)
edges_global = . %>% visEdges(shadow=TRUE, arrows='middle')
# layout_hierarchical = . %>% visHierarchicalLayout(
#   direction = 'LR', levelSeparation = 500) # direction = 'LR'
layout = . %>% visIgraphLayout(layout='layout.norm', layoutMatrix=igraph_layout)
options = . %>% visOptions(
  highlightNearest=list(enabled=TRUE, degree=1, hover=TRUE), 
  selectedBy=list(variable="id", multiple=TRUE),
  manipulation=TRUE)
interactive = . %>% visInteraction(
  keyboard=list(enabled=TRUE),
  multiselect=TRUE,
  navigationButtons=TRUE
)
configure = . %>% visConfigure(enabled=TRUE, showButton=TRUE)
legend = . %>% visLegend(
  enabled=TRUE,
  useGroups=FALSE
)

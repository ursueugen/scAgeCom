  ...
  dt_ctypes = dt[
    Category == COL_LIGAND_RECEPTOR_CELLTYPES
      ...
      c(COL_pval_readj, COL_pval_readj_UP, COL_pval_readj_DOWN) := .(
        ...
        stats::p.adjust(get(COL_pval_UP), method='BH'),
        stats::p.adjust(get(COL_pval_DOWN), method='BH')
      )
      ]
...
classify_edges <- function(dt_edge,
  ...
  edges_classification = purrr::pmap_chr(
    ...
    function(OR_DIFF, OR_UP, OR_DOWN, 
             pval_DIFF, pval_UP, pval_DOWN,
             pval_adj, pval_adj_UP, pval_adj_DOWN, 
             ...) {
      
      if (use_adjpval) {
        is_pval_UP = pval_adj_UP < 0.05
        is_pval_DOWN = pval_adj_DOWN < 0.05
      } else {
        is_pval_UP = pval_UP < 0.05
        is_pval_DOWN = pval_DOWN < 0.05
      }
...
add_edge_width <- function(G) {
  ...
  E(G)$width = rescale(sqrt(num_interacts), WIDTH_MIN, WIDTH_MAX)
...
setup_edges <- function(G, config, use_adjpval) {
  ...
  E(G)$edge.color = purrr::map_chr(E(G)$edge_type, map_edgetype_to_color)
  G = add_edge_width(G)
  return(G)
}
...
sort_layout_vertices = function(layout, G, config, disperse) {
  ...
  if(disperse) {
    ...
    new_layout[1:midpoint, 2][ligand_keys==0] = (
      new_layout[1:9, 2][ligand_keys==0] + scale_factor*vgap
    )
    new_layout[1:midpoint, 2][ligand_keys>0] = (
      new_layout[1:9, 2][ligand_keys>0] + 2*scale_factor*vgap
    )
  ...
  tryCatch(
    error = function(cnd) {
      return(list(
        ...
        ora = object@ORA,
        dt_filtered = object@cci_table_filtered
      )
      )
    },
